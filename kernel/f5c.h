...
/// @sa kstd
ideal f5cMain (
  ideal F,              ///<[in] intial ideal to compute the Groebner basis for
  ideal Q,              ///<[in] quotient ring
  tHomog h,             ///<[in] homogeneous or inhomogeneous?
  intvec ** mw,         ///<[in] weight vector for ordering
  intvec *hilb  = NULL, ///<[in] hilbert polynomial?
  int syzComp   = 0,    ///<[in] syzygy computation?
  int newIdeal  = 0,    ///<[in]
  intvec *vw    = NULL  ///<[in]
              );
...
ideal doF5  (
  ...
  ideal Q,        ///<[in] quotient ring
  intvec *w,      ///<[in] weight for ordering
  intvec *hilb,   ///<[in] hilbert polynomial?
  kStrategy strat ///<[in,out] strategy for fast reduction, etc.
            );
...
/// @brief \c initSF5 initializes the set \c strat->S which is used for generation of 
/// critical pair, reductions (in some cases), etc. This is a special version of 
/// \c initSSpecial from \c kutil.cc .
/// @sa initSSpecial
void initSF5  ( 
  ideal F,        ///<[in] initial ideal for Groebner basis computation
  ideal Q,        ///<[in] quotient ring
  ideal P,        ///<[in] temporary holder for polynomials to initialise S  
  kStrategy strat ///<[in,out] strategy for fast reduction, etc. 
              );



/// @brief \c redGlobalF5 is the variant of \c redBba for F5. 
/// It assumes a global ordering on reduces \c h using the set \c strat->S .
/// @return reduced \c h
/// @sa redBba
static poly redGlobalF5  (
  poly h,         ///<[in,out] element to be reduced    
  int maxIndex,   ///<[in] index in \c strat->S up to 
                  ///      which the reducers are taken
  kStrategy strat ///<[in] strategy for reduction
                      );



/// @brief \c redLocalF5 is the variant of \c redMora for F5. 
/// It assumes a local ordering on reduces \c h using the set \c strat->S .
/// @return reduced \c h
/// @sa redBba
static poly redLocalF5  (
  poly h,         ///<[in,out] element to be reduced    
  int maxIndex,   ///<[in] index in \c strat->S up to 
                  ///      which the reducers are taken
  kStrategy strat ///<[in] strategy for reduction
...
/// @brief \c enterPairsF5 enters the new, by \c h generated critical pairs to
/// strat. This is the F5-variant of \c enterpairsSpecial .
/// @sa enterpairsSpecial
void enterPairsF5 (
  poly h,           ///<[in] previously reduced w.r.t. \c strat, 
                    ///      pairs generated by \c h to be added to \c strat->S 
  int k,            ///<[in] index for 2nd generators of critical pairs 
  int ecart,        ///<[in] ecart of \c h
  int pos,          ///<[in] where to add the pair
  kStrategy strat,  ///<[in,out] strategy \c h is added to 
  int atR = -1      ///<[in] I have no idea!
                  );



/// @brief \c enterOnePairF5 enters the new, by \c h generated critical pairs to
/// strat. This is the F5-variant of \c enterOnePairSpecial . It enters the pair 
/// (s[i],p) into the set L, ecart=ecart(p) in the case that s forms a SB of (s).
/// @sa enterOnePairSpecial
void enterOnePairF5 (
  int i,            ///<[in] choose 2nd generator from this index onwards
  poly p,           ///<[in] 1st generator of critical pair
  int ecart,        ///<[in] ecart of \c p 
  kStrategy strat,  ///<[in,out] add critical pair to it 
  int atR = -1      ///<[in] I have no idea!
                    );



/// @brief \c pDivComp() is copied from \c kutil.cc .
/// This should be removed and the original one should be used in the end!
static inline int pDivComp  (
  poly p, ///<[in] 1st poly to be compared
  poly q  ///<[in] 2nd poly to be compared
                            );
